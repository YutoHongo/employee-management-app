...
namespace Backend.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        public ActionResult<Employee> GetEmployees()
        {
            ...
            // userIdがnullまたは整数型に変換出来ない場合Unauthorizedを返す
            if (string.IsNullOrEmpty(userIdClaim) || !int.TryParse(userIdClaim, out int userId))
                return Unauthorized("ユーザーIDが無効です");

            var employee = _employees.FirstOrDefault(e => e.Id == userId);

            // _employees内のIdにuserIdが無ければNotFoundを返す
            if (employee == null)
                return NotFound("該当する従業員情報がみつかりません");

            // e.Id == userIdに該当する要素（現在ログインしているユーザー情報のみを出力）
            return Ok(employee);
        }
    }
}